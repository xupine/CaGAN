...
def class_mask(args, model, iter, source_num_classes, target_num_classes, batch_size):
    ...
    c_s1 = torch.zeros(source_num_classes, batch_size, source_num_classes, 1, 1).cuda()
    c_t1 = torch.zeros(target_num_classes, batch_size, target_num_classes, 1, 1).cuda()
    c_t2 = torch.zeros(target_num_classes, batch_size, target_num_classes, 1, 1).cuda()

    # data
    source_set = potsdam.Potsdam(args, split='train', max_iters=None)
    target_set = panchromatic.Panchromatic(args, split='train', max_iters=None)
    ...
    for ii, sample in enumerate(sourceloader):
        ...
        for i in range(source_num_classes):
            if torch.all(source_features1[:,i] == 0):
                c_s1[i] = torch.zeros(batch_size, 6, 1, 1)
            else:
                source_features3[i] = source_features1[:,i]
                c_s1[i] = F.interpolate(source_features3[i], size=[1,1], mode='bilinear', align_corners=True)
        if ii == 0:
    ...
    for ii, sample in enumerate(targetloader):
        ...
        for i in range(target_num_classes):
            if torch.all(target_features1[:,i] == 0):
                c_t1[i] = torch.zeros(batch_size, 6, 1, 1)
            else:
                target_features3[i] = target_features1[:,i]
                c_t1[i] = F.interpolate(target_features3[i], size=[1,1], mode='bilinear', align_corners=True)
