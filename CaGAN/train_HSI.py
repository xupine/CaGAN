...
from Class_HSI import class_source, class_target
from class_mask_HSI import class_mask
from dataloaders.datasets import paviaU_S, paviaU_T
...
class Trainer(object):
    def __init__(self, args):
        ...
        # DATALOADERS
        # source
        self.source_train_set = paviaU_S.PaviaU_S(args, split='train', max_iters=args.max_iters * args.source_batch_size)
        self.source_val_set = paviaU_S.PaviaU_S(args, split='val', max_iters=None)
        ...
        self.source_val_loader = DataLoader(self.source_val_set, batch_size=args.source_batch_size, shuffle=False, drop_last=True,
                                            num_workers=args.workers, pin_memory=True)
        # target
        self.target_train_set = paviaU_T.PaviaU_T(args, split='train',max_iters=args.max_iters * args.target_batch_size)
        self.target_val_set = paviaU_T.PaviaU_T(args, split='val', max_iters=None)
    ...
    def training(self, epoch):
        ...
        c_ss = torch.randn(self.source_num_class, 1, 9, 4, 4,device='cuda')
        c_tt = torch.randn(self.source_num_class, 1, 9, 4, 4,device='cuda')
        self.model.train()
        self.d_h.train()
        ...
        for iter_i in tqdm(range(self.args.num_steps)):
            ...
            pred_src_main1 = pred_src_main1/2
            ...
            c_tb = torch.zeros(self.target_num_class, self.args.target_batch_size, 9, 4, 4).cuda()
            c_tb1 = torch.zeros(self.target_num_class, self.args.target_batch_size, 9, 4, 4).cuda()
            # target
            ...
            for i in range(self.target_num_class):
                ...
                if torch.all(target_features3 == 0):
                    c_tb1[i] = torch.zeros(self.args.target_batch_size, 9, 4, 4)
                else:
                    c_tb1[i] = F.interpolate(target_features3, size=[4,4], mode='bilinear', align_corners=True)
            if iter_i == 0:
            ...
            #source
            source_features3 = torch.FloatTensor(self.source_num_class, self.args.source_batch_size, self.source_num_class, 128, 128)
            c_sb = torch.zeros(self.source_num_class, self.args.source_batch_size, 9, 4, 4).cuda()
            c_sb1 = torch.zeros(self.source_num_class, self.args.source_batch_size, 9, 4, 4).cuda()
            ...
            for i in range(self.source_num_class):
                ...
                if torch.all(source_features3[i] == 0):
                    c_sb1[i] = torch.zeros(self.args.source_batch_size, 9, 4, 4)
                else:
                    c_sb1[i] = F.interpolate(source_features3[i], size=[4,4], mode='bilinear', align_corners=True)
            if iter_i == 0:
            ...
            as_11 = torch.zeros(pred_tar_main.size()[0], 16, 16).cuda()
            at_11 = torch.zeros(pred_tar_main.size()[0], 16, 16).cuda()
            a_1 = torch.zeros(pred_tar_main.size()[0], 16, 16).cuda()

            as_22 = torch.zeros(pred_tar_main.size()[0], 16, 16).cuda()
            at_22 = torch.zeros(pred_tar_main.size()[0], 16, 16).cuda()
            a_2 = torch.zeros(pred_tar_main.size()[0], 16, 16).cuda()
    ...
    def validation_source(self, i_iter, epoch, c_ss, c_tt):
        ...
        F1_mean1 = (F1[0] + F1[1] + F1[2] + F1[3] + F1[4] + F1[5]+ F1[6]+ F1[7]+ F1[8]) / 9.0
        mIoU1 = (IoU[0] + IoU[1] + IoU[2] + IoU[3] + IoU[4] + IoU[5] + IoU[6] + IoU[7] + IoU[8]) / 9.0
        ...
        print("F1[0]:{}, F1[1]:{}, F1[2]:{}, F1[3]: {}, F1[4]: {}, F1[5]: {}, F1[6]: {}, F1[7]: {}, F1[8]: {}, F1_mean: {}, F1_mean1: {}".format(F1[0],F1[1],F1[2],F1[3],F1[4],F1[5],F1[6],F1[7],F1[8],F1_mean,F1_mean1))
        print("IoU[0]:{}, IoU[1]:{}, IoU[2]:{}, IoU[3]: {}, IoU[4]: {}, IoU[5]: {}, IoU[6]: {}, IoU[7]: {}, IoU[8]: {}, mIoU: {}, mIoU1: {}".format(IoU[0],IoU[1],IoU[2],IoU[3],IoU[4],IoU[5],IoU[6],IoU[7],IoU[8],mIoU,mIoU1))
    ...
    def validation_target(self, i_iter, epoch, c_ss, c_tt):
        ...
        file11 = "./IoU6.txt"
        with open(file11, 'a', encoding='utf-8') as f10:
            f10.writelines(str(IoU[6]) + '\n')
        file12 = "./IoU7.txt"
        with open(file12, 'a', encoding='utf-8') as f11:
            f11.writelines(str(IoU[7]) + '\n')
        file13 = "./IoU8.txt"
        with open(file13, 'a', encoding='utf-8') as f12:
            f12.writelines(str(IoU[8]) + '\n')
