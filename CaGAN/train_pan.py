...
from Class_pan import class_source, class_target
from class_mask_pan import class_mask
from dataloaders.datasets import potsdam, panchromatic
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.source_val_loader = DataLoader(self.source_val_set, batch_size=args.source_batch_size, shuffle=False, drop_last=True,
                                            num_workers=args.workers, pin_memory=True)
        # target
        self.target_train_set = panchromatic.Panchromatic(args, split='train',max_iters=args.max_iters * args.target_batch_size)
        self.target_val_set = panchromatic.Panchromatic(args, split='val', max_iters=None)
    ...
    def training(self, epoch):
        ...
        #c_ss = torch.randn(self.source_num_class, 1, 6, 1, 1,device='cuda')
        #c_tt = torch.randn(self.source_num_class, 1, 6, 1, 1,device='cuda')
        self.model.train()
        self.d_h.train()
        ...
        for iter_i in tqdm(range(self.args.num_steps)):
            ...
            c_tb = torch.zeros(self.target_num_class, self.args.target_batch_size, 6, 1, 1).cuda()
            c_tb1 = torch.zeros(self.target_num_class, self.args.target_batch_size, 6, 1, 1).cuda()
            # target
            ...
            for i in range(self.target_num_class):
                ...
                if torch.all(target_features3 == 0):
                    c_tb1[i] = torch.zeros(self.args.target_batch_size, 6, 1, 1)
                else:
                    c_tb1[i] = F.interpolate(target_features3, size=[1,1], mode='bilinear', align_corners=True)
            if iter_i == 0:
            ...
            c_sb = torch.zeros(self.source_num_class, self.args.source_batch_size, 6, 1, 1).cuda()
            c_sb1 = torch.zeros(self.source_num_class, self.args.source_batch_size, 6, 1, 1).cuda()
            ...
            for i in range(self.source_num_class):
                ...
                if torch.all(source_features3[i] == 0):
                    c_sb1[i] = torch.zeros(self.args.source_batch_size, 6, 1, 1)
                else:
                    c_sb1[i] = F.interpolate(source_features3[i], size=[1,1], mode='bilinear', align_corners=True)
            if iter_i == 0:
