...
import numpy as np
from dataloaders.datasets import paviaU_T, paviaU_S
...
def class_mask(args, model, iter, source_num_classes, target_num_classes, batch_size):
    model.eval()
    mask_source = np.zeros((source_num_classes, 128, 128))
    #mask_source1 = torch.FloatTensor(source_num_classes, 128, 128)
    mask_target = np.zeros((target_num_classes, 128, 128))
    #mask_target1 = torch.FloatTensor(source_num_classes, 128, 128)

    source_features = torch.FloatTensor(source_num_classes, source_num_classes, 128, 128).cuda()
    source_features1 = torch.FloatTensor(batch_size, source_num_classes, source_num_classes, 128, 128).cuda()
    source_features3 = torch.FloatTensor(source_num_classes, batch_size, source_num_classes, 128, 128)

    target_features = torch.FloatTensor(target_num_classes, target_num_classes, 128, 128).cuda()
    target_features1 = torch.FloatTensor(batch_size, target_num_classes, target_num_classes, 128, 128).cuda()
    target_features3 = torch.FloatTensor(target_num_classes, batch_size, target_num_classes, 128, 128)

    c_s1 = torch.zeros(source_num_classes, batch_size, source_num_classes, 4, 4).cuda()
    c_t1 = torch.zeros(target_num_classes, batch_size, target_num_classes, 4, 4).cuda()
    c_t2 = torch.zeros(target_num_classes, batch_size, target_num_classes, 4, 4).cuda()

    # data
    source_set = paviaU_T.PaviaU_T(args, split='train', max_iters=None)
    target_set = paviaU_S.PaviaU_S(args, split='train', max_iters=None)
    ...
    for ii, sample in enumerate(sourceloader):
        ...
        for jj in range(source_feature.size()[0]):
            ...
            source_l = np.zeros((128, 128), dtype=np.float32)
        ...
        for i in range(source_num_classes):
            if torch.all(source_features1[:,i] == 0):
                c_s1[i] = torch.zeros(batch_size, 9, 4, 4)
            else:
                source_features3[i] = source_features1[:,i]
                c_s1[i] = F.interpolate(source_features3[i], size=[4,4], mode='bilinear', align_corners=True)
        if ii == 0:
    ...
    for ii, sample in enumerate(targetloader):
        ...
        for jj in range(target_feature.size()[0]):
            ...
            target_l = np.zeros((128, 128), dtype=np.float32)
        ...
        for i in range(target_num_classes):
            if torch.all(target_features1[:,i] == 0):
                c_t1[i] = torch.zeros(batch_size, 9, 4, 4)
            else:
                target_features3[i] = target_features1[:,i]
                c_t1[i] = F.interpolate(target_features3[i], size=[4,4], mode='bilinear', align_corners=True)
