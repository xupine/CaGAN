...
def mix_rbf_mmd2(source, target, biased=True):
    sigma_list = [1,2,5,10]
    ...
    for ii in range(num_class):
        ...
        K_XX, K_XY, K_YY, d = _mix_rbf_kernel(source1, target1, sigma_list)
    # return _mmd2(K_XX, K_XY, K_YY, const_diagonal=d, biased=biased)
        loss = _mmd2(K_XX, K_XY, K_YY, const_diagonal=False, biased=biased)
        loss_a += F.relu(loss)
    return loss_a/num_class

def mix_rbf_mmd2d(source, target, biased=True):
    sigma_list = [1,2,5,10]
    num_class = int(source.size()[0])
    loss_a = 0.0
    for ii in range(num_class):
