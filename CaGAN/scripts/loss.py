...
class Losses(object):
    ...
    def Diff2d_dclass(self, inputs1, inputs2):
        loss = 0
        inputs1 = inputs1.view(inputs1.size(0),-1)
        inputs2 = inputs2.view(inputs2.size(0), -1)
        for i in range(self.num_class):
            for j in range(i+1,self.num_class):
                loss  += torch.mean(torch.abs(inputs1[i]-inputs2[j]))
        return loss/((self.num_class*(self.num_class-1))/2)
